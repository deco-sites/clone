...
function SectionItem({ item }: { item: Item }) {
  return (
    <span class="text-white hover:text-green-500">
      {isIcon(item)
        ? (
...
function FooterContainer(
  ...
  return (
    <div class={`py-6 md:py-20 md:pb-[50px] px-4 sm:py-12 sm:px-0 ${_class}`}>
      {children}
    </div>
  );
}
...
function SpotifyFooter(
  { sections = [], logo, extraLinks, socialMedia }: Props,
) {
  return (
    <footer class="w-full bg-black flex flex-col">
      <div>
        <div class="container w-full flex flex-col">
          <FooterContainer class="max-w-[1170px] mx-auto md:flex md:flex-wrap md:justify-between">
            <div className="md:max-w-[89px] md:w-[89px] flex">
              {logo && (
                <Image
                  src={logo.url}
                  alt={logo.alt}
            ...
            <ul class="flex md:flex-row flex-col gap-[50px] justify-center md:w-1/2 md:justify-between">
              {sections.map((section) => (
                <li>
                  <div>
                    <span class="font-bold text-xs text-[#919496] uppercase tracking-[0.08em]">
                      {section.label}
                    </span>
            ...
            {socialMedia && (
              <ul className="flex gap-[15px] mt-[120px] mb-[15px] md:w-fit md:mt-5">
                {socialMedia.map((link) => (
                  <li className="">
                    <a
                      href={link.link}
                      className="flex w-11 h-11 md:w-[54px] md:h-[54px] bg-[#222326] rounded-full items-center justify-center"
                    >
                      <Image
                        src={link.image.url}
                        alt={link.image.alt}
                        width={24}
                        className="bg-white h-6 w-6 hover:bg-green-500"
                      />
                    </a>
                  </li>
                ))}
            ...
            <div className="flex flex-col items-end md:w-full">
              <a href="javascript:void(0)" className="flex items-center mb-4">
                <svg
                  width="12"
                  height="13"
                  viewBox="0 0 12 13"
                ...
                <p className="text-[10px] text-[#919496] hover:text-green-500">
                  Brasil (português)
                </p>
              </a>
            </div>
            ...
            {extraLinks && (
              <div className="flex justify-between md:w-full">
                <ul className="flex gap-y-2 gap-x-6 flex-wrap pr-[45px]">
                  {extraLinks.map((link) => (
                    <li className="flex">
                      <a
                        className="text-[10px] md:text-xs text-[#919496] hover:text-green-500"
                        href={link.url}
                      >
                        {link.label}
                      </a>
                ...
                <span className="flex text-[10px] text-[#919496] whitespace-nowrap md:items-center">
                  © 2023 Spotify AB
                </span>
              </div>
            )}
